...
export interface TypedSearchFilterTypes {${config.recordIds.map(id => {
    return `
...
export interface TypedSearchColumnTypes {${config.recordIds.map(id => {
    return `
${indent()}${id}: ${id}SearchColumnTypes;
`.trim()
  }).join(`\n${indent()}`)}
}`.trim()
  return { output }
...
export interface TypedSearchColumnNames {${config.recordIds.map(id => {
    return `
${indent()}${id}: Required<${id}SearchColumnNames>;
`.trim()
  }).join(`\n${indent()}`)}
}`.trim()
  return { output }
...
function generateTypedSearchFilterNames(config: ProjectConfig & { recordIds: string[] }) {
  ...
  export interface TypedSearchFilterNames {${config.recordIds.map(id => {
    return `
  ${indent()}${id}: Required<${id}SearchFilterNames>;
  `.trim()
  }).join(`\n${indent()}`)}
  }`.trim()
  return { output }
...
function generateTypedSearchJoinTypes(config: ProjectConfig & { recordIds: string[] }) {
  const output = `
    ...
    export interface TypedSearchJoinTypes {${config.recordIds.map(id => {
    return `
