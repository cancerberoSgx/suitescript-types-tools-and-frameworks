define(["require", "exports", "./typedRecordOperations"], function (require, exports, typedRecordOperations_1) {
    ...
    function typedRecordTest() {
        var c2 = typedRecordOperations_1.load({ id: 33, type: 'commercecategory' });
        if (!c2)
            return;
        var v21 = c2.getValue({ fieldId: 'primaryparent' }); // / must be string | undefined
        var v22 = c2.getValue({ fieldId: 'created' }); // / must be Date | undefined
        var v23 = c2.getValue('primaryparent'); // must be string | undefined
        var s2s = c2.getSublists(); // is ("items" | "subcategories" | "urls")[] which is not exactly correct
        var fields = c2.getFields(); // is ("addtohead" | "catalog" | "created" | "description" | "displayinsite" | "externalid" | "isinactive" | "lastmodified" | "lastmodifiedby" | "metadescription" | "metakeywords" ...
        // s2s[44]
