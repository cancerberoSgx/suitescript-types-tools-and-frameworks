...
export function fixDefine(s: string, names: string[], thisName: string): string {
  s = s.split('\n').map(l => {
    ...
    // l = l.replace('</script>', `define("${thisName}", [`)
    // if(l.indexOf('</scrip')!==-1){
    //     console.log(l);
...
export function renderWithAmdFiles(e: JSX.Element, config: RenderConfig & { asHtmlDocument?: boolean, files: File[] }): string {

  const s = `<script>
${almond()}
${config.files.map(f => fixDefine(readFileSync(f.path).toString(), config.files.map(f => f.name), f.name)).join(';\n\n')}
</script>`

  const s2 = ReactLike.render(e, { renderClientCode: true })

  return config.asHtmlDocument ? `
<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8" />
