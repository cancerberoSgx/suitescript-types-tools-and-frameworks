...
/** 
 * Payment Item Fields Definition.
 * Record's Internal Id: paymentitem. 
 * Supports Custom Fields: true 
 */
export interface paymentitemFields {
...
Go to <_TABNAME=LIST_LOCATION_> > <_TASKCATEGORY=LIST_LOCATION_> > Locations for details about existing location records. */
  location?: string;
  /** Select a payment method to associate with this payment item.

The payment method you choose here is for reference only. It does not initiate a payment transaction.

To add choices to this list, go to <_TABNAME=LIST_ACCOUNTINGOTHERLIST_> > Accounting > Accounting Lists > New > Payment Method. */
  paymentmethod?: string;
  /**  */
  subsidiary?: string;
  /** 1. Click the button after Group with Undeposited Funds if you want these payments to be processed as any other undeposited funds.
   2. If these are to be deposited immediately into an account, click the button between Deposit To and Account. */
  undepfunds?: string;
}
...
/** 
 * Payment Item Sublists Definition.
 * Record's Internal Id: paymentitem. 
 * Supports Custom Fields: true 
 */
export interface paymentitemSublists {
